...
def get_user_settings(user_id):
    conn = sqlite3.connect('user_settings.db')
    cursor = conn.cursor()
    cursor.execute('''SELECT selected_voice, selected_speed, format, role FROM user_settings WHERE user_id = ?''',
                   (user_id,))
...
def save_referral_invited(referral_id, user_id):
    ...
    cursor.execute("UPDATE ref_table SET count_request_month = IFNULL(count_request_month, 0) + ? WHERE user_id = ?",
                   (bonus_ref, referral_id))
    conn.commit()
...
def add_user(user_id, name, subscription_date, count_symbol, request_month, unlimited, status, role):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO my_users (user_id, name, subscription_date, count_symbol, request_month, unlimited,status,role ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (user_id, name, subscription_date, count_symbol, request_month, unlimited, status, role))
        conn.commit()
    finally:
...
def insert_or_update_user(username):
    ...
    if user_exists:
        ...
        cursor.execute("UPDATE activity_today SET last_activity_date = ? WHERE user_id = ?",
                       (current_datetime, username))
    else:
        # Если пользователь не существует, вставляем новую запись
        cursor.execute("INSERT INTO activity_today (user_id, last_activity_date) VALUES (?, ?)",
...
async def add_daily_requests():
    logging.info("Запуск функции add_daily_requests")

    async with aiosqlite.connect('users.db') as conn:
        cursor = await conn.cursor()
        await cursor.execute("SELECT count FROM bonus_day")
        count = await cursor.fetchall()

        if count:
            await cursor.execute("UPDATE my_users SET request_month = ?", (count[0][0],))
        else:
            await cursor.execute("UPDATE my_users SET request_month = ?", (25,))

        await conn.commit()
        logging.info("Функция add_daily_requests завершена успешно")
...
def get_count_symbol(user_id):
    conn = sqlite3.connect('users.db')
    cursor = conn.cursor()
    data = cursor.execute("SELECT count_symbol  FROM my_users WHERE user_id = ?", (user_id,))
    result = data.fetchone()
    conn.close()
    return result[0]

def get_count_symbol_all():
    try:
        conn = sqlite3.connect('users.db')
        cursor = conn.cursor()
        data = cursor.execute("SELECT count_symbol  FROM my_users WHERE id = 4")
        result = data.fetchone()
        conn.close()
        return result[0]
    except Exception as e:
